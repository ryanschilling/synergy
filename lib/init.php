...
function roots_setup() {
  ...
  add_filter('mce_buttons_2', 'roots_add_mce_styles_button');
  add_filter('tiny_mce_before_init', 'roots_add_mce_styles');
}
add_action('after_setup_theme', 'roots_setup');

function roots_add_mce_styles_button($buttons){
  $first_button = array_shift($buttons);
  array_unshift($buttons, $first_button, 'styleselect');
  return $buttons;
}

function roots_add_mce_styles($args){
  $args['theme_advanced_styles'] = 'Lead Text=lead;Small Text=small';
  return $args;
}